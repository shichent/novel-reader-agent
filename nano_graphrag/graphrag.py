...
from .MinHash import HashTable, MinHash, jaccard_similarity
...
class GraphRAG:
    ...
    def __post_init__(self):
        self.K,self.L,self.B, self.R = 2,50,512,2**20
        ...
        # LSH
        self.hashtable = HashTable(self.K,self.L,self.B,self.R)
        internal_graph = self.chunk_entity_relation_graph._graph
        for node, data in internal_graph.nodes(data=True):
            hashcodes = MinHash(node, self.hashtable.K*self.hashtable.L)
            self.hashtable.insert(hashcodes, node)
