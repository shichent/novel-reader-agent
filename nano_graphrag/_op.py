...
async def _build_local_query_context(
    ...
    if query_param.use_LSH:
        hashcodes = MinHash(query,hashtable.K*hashtable.L)
        results = hashtable.lookup(hashcodes)
        if len(results)>query_param.top_k:
            results_scored = []
            for r in results:
                sim = jaccard_similarity(r,query,1)
                results_scored.append((sim,r))
            results_scored.sort(reverse=True)
            results = [{
                'entity_name': r
            } for s,r in results_scored[:query_param.top_k]]
    else:
